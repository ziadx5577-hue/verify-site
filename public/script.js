...
async function init() {
  ...
  document.getElementById("rating").textContent = Number(stats.rating).toFixed(2);
  document.getElementById("reviewCount").textContent = `${stats.reviewCount}+`;
  document.getElementById("purchaseCount").textContent = stats.purchaseCount;
  const me = await fetch("/api/me").then((response) => response.json());
  if (me.loggedIn) document.getElementById("discordLogin").textContent = `مسجل: ${me.user.username}`;
}
